...
/**
 * The editor is a reflection from PT Plugin.
 * However, it has its own tricks (since it has its own challenges!)
 * 
 * Biggest challenge is that the directive are single line 
 * comments. So, all we need to do is :
 * 1. Let all the Pyjama specific keywords, grammar, etc be recognised in 
 * the standard way.
 * 2. Treat the single line comments specially to identify the directives
 * 
 * Identifying the directives can lead to them being syntax coloured, auto-completed, etc.
 *
 * @author vikassingh
 */
public class CodeScanner extends AbstractJavaScanner {

	/**
	 * Reflective of grammar files from compiler code
	 * Originally, OpenMP 2.5 directive keywords
	 */
	private static String[] mPyjamaTokens = { 
//	    "//#omp",
	    "parallel",
	    "firstprivate",
	    "lastprivate",
	...
	private static class VersionedWordMatcher extends CombinedWordRule.WordMatcher implements ISourceVersionDependent {
		...
		private final IToken mDefaultToken;
		private final String mVersion;
		private boolean mbIsVersionMatched;
		...
		public void setSourceVersion(String version) {
			mbIsVersionMatched= mVersion.compareTo(version) <= 0;
		}
		...
		public IToken evaluate(ICharacterScanner scanner, CombinedWordRule.CharacterBuffer word) {
			...
			if (mbIsVersionMatched || token.isUndefined())
				return token;
	...
	private static class AnnotationRule implements IRule, ISourceVersionDependent {
		...
		private static final class ResettableScanner implements ICharacterScanner {
			private final ICharacterScanner mDelegate;
			...
			public char[][] getLegalLineDelimiters() {
				return mDelegate.getLegalLineDelimiters();
			}
		...
		private final IWhitespaceDetector mWhitespaceDetector= new JavaWhitespaceDetector();
		private final IWordDetector mWordDetector= new JavaWordDetector();
		private final IToken mInterfaceToken;
		private final IToken mAtToken;
		private final String mVersion;
		private boolean mbIsVersionMatch;
		...
		private boolean readInterface(ICharacterScanner scanner) {
			...
			if (mWordDetector.isWordPart((char) ch))
				return false;
		...
		private boolean skipWhitespace(ICharacterScanner scanner) {
			while (mWhitespaceDetector.isWhitespace((char) scanner.read())) {
				// do nothing
			}
		 ...
		 */
		public void setSourceVersion(String version) {
			mbIsVersionMatch= mVersion.compareTo(version) <= 0;
		}
	...
	private static String[] mJavaDataTypes = { 
		"void", 
		"boolean", 
		"char", 
		"byte", 
	 ...
	 */
	public CodeScanner(IColorManager manager, IPreferenceStore store) {
		super(manager, store);
		initialize();
	}
	...
	protected List createRules() {
		...
		token= getToken(IJavaColorConstants.JAVA_KEYWORD);
		for (int i=0; i<mJavaKeywords.length; i++)
			wordRule.addWord(mJavaKeywords[i], token);
		for (int i=0; i<mJavaDataTypes.length; i++)
			wordRule.addWord(mJavaDataTypes[i], token);
		for (int i=0; i<fgConstants.length; i++)
			wordRule.addWord(fgConstants[i], token);

		//  Pyjama data clauses are added as keywords
		token= getToken(IJavaColorConstants.JAVA_KEYWORD);
